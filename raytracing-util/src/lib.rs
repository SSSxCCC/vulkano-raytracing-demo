...
use glam::Affine3A;
use std::sync::Arc;
use vulkano::VulkanoContextExtension;
use vulkano_util::context::VulkanoContext;

pub struct RenderContext {
    ash: AshContext,
    vulkano_ext: VulkanoContextExtension,
    vulkano: VulkanoContext,
}

impl RenderContext {
    pub fn new(vulkano: VulkanoContext) -> Self {
        RenderContext {
            ash: AshContext::new(&vulkano),
            vulkano_ext: VulkanoContextExtension::new(&vulkano),
            vulkano,
        }
    }

    pub fn ash(&self) -> &AshContext {
        &self.ash
    }

    pub fn vulkano_ext(&self) -> &VulkanoContextExtension {
        &self.vulkano_ext
    }

    pub fn vulkano(&self) -> &VulkanoContext {
        &self.vulkano
    }
}
